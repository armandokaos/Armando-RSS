...
export async function publishPressRelease(options: PublishOptions): Promise<string> {
  ...
  const { to, data } = (await result.json()) as { to: string; data: string };
